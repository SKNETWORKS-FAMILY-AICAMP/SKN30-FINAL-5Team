...
/**
 * The server's own per-agent reasoning, shown as the agents reported it.
 *
 * An earlier pass replaced this with three client-side buckets (goal /
 * condition / environment) derived by regex-matching decision reason codes.
 * That reproduced backend decision logic in the client, which AGENTS.md 6
 * forbids, and it mislabelled codes: SAFETY_TIME_LIMIT read as a goal reason
 * and the safety summary's codes were folded into a neutral condition bucket.
 * The server already publishes public_agent_summaries for exactly this.
 */
