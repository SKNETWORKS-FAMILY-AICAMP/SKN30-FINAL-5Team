...
| pain alternative | source/target을 `exercises.id` FK로 해석하고 `reason_code=DISCOMFORT`, `pain_discomfort_area_code`, `condition_code`, `service_action_code`, `target_strategy_code`, direction·source metadata를 적재 | 현재 nullable selector는 migration `0028_discomfort_alternative_conditions` 대상이다. migration 적용·승인은 백엔드 범위이며 이 문서는 요구사항만 인계함 |
