...
def test_real_catalog_is_fully_covered_by_direct_or_manual_decisions(tmp_path: Path) -> None:
    ...
    assert all(set(row["safety_relevant_body_area_codes"].split("|")) <= allowed for row in rows)
...
def test_direct_source_values_are_preserved(tmp_path: Path) -> None:
    ...
    source_by_id = {row["id"]: row for row in json.loads(SOURCE.read_text(encoding="utf-8"))}
    with output.open(encoding="utf-8-sig", newline="") as handle:
        rows = list(csv.DictReader(handle))
    for row in rows:
        if row["source_identity"] in source_by_id:
            expected = source_by_id[row["source_identity"]]["exercise_contraindicated_pain_regions"]
