...
catalog = load_script("build_catalog_enrichment_v3_fitt", "build_catalog_enrichment_v3_fitt.py")
...
class FittTemplatePolicyTests(unittest.TestCase):
    def test_beginner_defaults_and_units(self) -> None:
        ...
        self.assertEqual(
            (
                by_id["FITT-COMPOUND-SQUAT-V1"]["default_sets"],
                by_id["FITT-COMPOUND-SQUAT-V1"]["default_reps"],
            ),
            ("2", "10"),
        )
    ...
    def test_exercise_specific_mapping_keeps_equipment_out_of_exercise_group(self) -> None:
        ...
        self.assertEqual(
            catalog.choose_template("NEX-000118", "ISOLATION")[0], "FITT-ISOLATION-STRENGTH-V1"
        )
        self.assertEqual(
            catalog.choose_template("NEX-000162", "HINGE")[0], "FITT-COMPOUND-HINGE-V1"
        )
        self.assertEqual(catalog.choose_template("NEX-000208", "HINGE")[0], "FITT-HINGE-POWER-V1")
        self.assertEqual(
            catalog.choose_template("NEX-000113", "PUSH")[0], "FITT-ISOMETRIC-STRENGTH-V1"
        )
    ...
    def test_intermediate_core_dynamic_uses_10_to_15_reps(self) -> None:
        source = next(
            row for row in beginner.TEMPLATES if row["fitt_template_id"] == "FITT-CORE-DYNAMIC-V1"
        )
