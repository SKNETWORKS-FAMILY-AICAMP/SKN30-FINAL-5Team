...
1. 테스트 로그인, 생년월일(`YYYY-MM-DD`) 입력, 사용자 timezone의 로컬 날짜 기준 서버 만 18–64세 eligibility 판정, 생년월일 암호화 저장, 생년월일과 만 나이는 인증된 본인의 프로필 조회 응답에만 반환하고 에이전트·decision 입력에는 전달하지 않음(ADR-0020)
2. 홈·맨몸·상체 목표 온보딩
3. 검수된 40분 기본 루틴
