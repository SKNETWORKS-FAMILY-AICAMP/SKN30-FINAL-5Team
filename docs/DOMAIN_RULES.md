...
- 기본 경험은 추천 옵션 하나와 더 가벼운 옵션 하나를 제공한다.
- REST와 STOP_AND_SEEK_HELP에는 운동 계획을 제공하지 않는다.
- RECOVERY의 추천 옵션은 검수된 저강도 회복 계획이며, 더 가벼운 옵션은 완전 휴식일 수 있다.
- 웨어러블 데이터가 없어도 모든 핵심 흐름이 동작한다.
- 웨어러블 데이터만으로 안전 결정을 내리지 않는다.
- 안전 거부는 조정 에이전트와 LLM이 무시할 수 없다.
- 사용자가 원래 계획을 선택할 수 있는지는 서버의 최종 안전 검증 결과로 결정한다.
...
- primary_plan은 null이다.
- lighter_plan은 null이다.
...
REST에는 primary_plan과 lighter_plan을 모두 제공하지 않는다.
...
운동 시작 후 `actual_elapsed_seconds`는 0초부터 증가하는 클라이언트 경과 타이머 값이다. 이 값은 수행량 확인과 기록에만 사용하며 운동 블록, 세션 상태 또는 안전 결과를 자동 판정하지 않는다.

---

## 6. 요청 시간 보존형 다운시프트

다운시프트는 사용자의 requested duration을 유지하면서 수행 부담을 낮춘다. primary와 lighter는 원칙적으로 같은 requested duration을 가진다.
...
7. 추천안과 더 가벼운 옵션을 각각 최종 안전 검증한다.
...
- 1년 이상 서비스 활동 없음: 계정 운영상 `DORMANT` 후보이며, 통지·재활성화·삭제 정책은 개인정보 검토 후 확정
...
전문 에이전트는 독립된 구조화 proposal을 반환한다.
...
proposal_status는 `READY`, `NEEDS_INPUT`, `FAILED` 중 하나다. opaque confidence 점수는 MVP 계약에 포함하지 않는다. 필수 에이전트 네 개 중 하나라도 `FAILED`이거나 proposal이 누락되면 decision run은 `FAILED`이며 계획을 성공 응답하지 않는다.

evidence는 저장된 입력 필드와 규칙 코드를 가리키는 구조화 참조다. 내부 추론 전문을 저장하거나 클라이언트에 노출하지 않는다.

조정기는 전문 에이전트가 만든 제안과 이미 생성된 후보를 입력받아 primary option과 lighter option을 선택한다. 조정기는 카탈로그 밖 운동을 생성하거나 안전 veto를 제거할 수 없다.
...
- primary, lighter, original 또는 rest 중 서버가 selectable로 표시한 옵션만 선택할 수 있다.
- 안전 veto된 original 옵션은 selectable=false다.
- KEEP, DOWNSHIFT, CHANGE, RECOVERY 결정에서는 사용자가 운동 대신 쉴 수 있도록 별도의 REST 옵션을 제공한다. 이 옵션은 lighter plan이 아니라 사용자 override다.
- REST 권고에는 운동 옵션이 없다.
- STOP_AND_SEEK_HELP에는 선택 가능한 운동 옵션이 없다.
- RECOVERY의 lighter 옵션은 REST일 수 있다.
...
| 선택 데이터 누락 | unknown으로 유지하고 의료 상태를 추론하지 않음 |
| LLM 장애 | 템플릿 설명 사용 |
