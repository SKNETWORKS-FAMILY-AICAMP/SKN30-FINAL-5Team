...
- 휴식 타이머는 현행 계약상 count-up이다. 작은 `휴식 경과` 라벨 아래 기존 56px의 `mm:ss`를 분리해 중앙 정렬한다. 임의의 제한 시간이나 자동 종료를 추가하지 않는다.
...
- 사용자 추가 지시: `상체 근력 루틴` 줄을 제거하고 `상체 · 근력 · 40분` 정보만 남긴다.
- 실제 앱은 기존 API final_plan의 `body_focus_code`, `training_type_code`, `requested_duration_minutes`를 그대로 표시한다. 프리뷰는 같은 형태의 샘플 응답을 사용한다.
- 변경 파일: `HomeRoutineCard.tsx`, `HomeScreen.test.tsx`, `PreviewGallery.test.tsx`, 이 작업 기록. 기존 작업 브랜치/worktree와 통합 원본에 같은 변경을 적용하고 다른 작업의 staged 변경은 보존했다.
- 검증: 통합 원본에서 홈·프리뷰 2 suites / 101 tests, 포맷·타입 검사, Android/iOS production export 통과. 린트 오류 0개, 기존 BirthDateField 의존성 경고 1개. 브라우저 `8082`의 Home(API) → 최종 추천에서 루틴명 없이 부위·유형·시간 한 줄만 표시됨을 확인했다.
- API·DB·개인정보 처리 변경 없음. 실제 서버·실기기 검증은 별도다.

## 후속 수정: GIF 로딩과 휴식 시간 위계

- 임시 마스코트는 GIF 로딩 중 또는 실패 시에만 렌더링한다. 로드 성공 후 투명한 GIF 뒤로 정적 이미지가 겹치지 않도록 제거한다.
- `휴식 경과`는 13px, 시간은 기존 56px를 유지하고 각각 중앙 정렬한다.
- 변경 파일: `WorkoutScreen.tsx`, `WorkoutScreen.test.tsx`, 이 작업 기록. API·DB·안전 판정·타이머 로직과 개인정보 처리는 변경하지 않는다.
- 수동 확인: GIF 로드 후 정적 마스코트가 사라지는지, 로딩 실패 시 대체 이미지가 보이는지, 휴식 화면에서 작은 라벨과 큰 시간이 중앙에 정렬되는지 확인한다.
- 후속 검증: `WorkoutScreen.test.tsx` 72개 테스트, 포맷·타입 검사, Android/iOS production export 통과. 린트 오류 0개 및 기존 BirthDateField 경고 1개. 브라우저에서 GIF 로드 완료·임시 이미지 0개와 휴식 라벨/시간의 크기·중앙 정렬을 확인했다. 실기기 검증은 별도로 필요하다.
