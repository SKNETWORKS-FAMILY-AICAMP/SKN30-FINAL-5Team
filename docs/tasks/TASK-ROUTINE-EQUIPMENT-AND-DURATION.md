...
| # | 항목 | 필요한 결정 |
|---|---|---|
| 1 | `LOCATION` 관계(35건) | 변형운동에 포함할지, 제외할지 |
| 2 | 복귀 판정 기준 | 현재는 `COMPLETION_GAP_14_DAYS`(운동 완료 공백). 결정은 "14일 미접속"이므로 접속 기록 기준으로 바꾸려면 판정 로직 변경이 필요하다. 안전 규칙이므로 개발팀장 확인 대상 |
| 3 | 레벨별 적합성 표현 | `beginner_suitable` 불리언 하나로는 "중급에게 권장"을 담을 수 없다. 검수 4단계(`SUITABLE`/`CONDITIONAL`/`NOT_PRIORITY`/`REVIEW_REQUIRED`)를 보존하는 컬럼이 필요하며 마이그레이션 대상. 중급 오픈 시점에 함께 처리 |
