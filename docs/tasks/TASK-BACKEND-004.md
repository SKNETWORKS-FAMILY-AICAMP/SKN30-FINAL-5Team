...
> 상태: `SUPERSEDED` — 이 문서의 만 14세 기준과 기존 동의 범위는 최신 서비스 정책의 만 18–64세
> eligibility·개인정보 consent 네 코드·별도 이용약관 동의 이력 계약으로 대체됐다. 생년월일 암호화와
> 사용자 timezone 기준 서버 검증 원칙만 후속 구현에서 재사용한다.
