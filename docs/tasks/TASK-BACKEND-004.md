...
백엔드 경계를 만든다. 서버가 사용자 timezone의 로컬 날짜로 만 14세 적격성을 판정하고,
생년월일 원문과 파생 나이가 불필요한 계층으로 확산되지 않게 한다.

## 이번 착수 범위

- 사용자 timezone 기준 만 나이와 만 14세 적격성 계산
...
5. 윤년 2월 29일 출생자의 비윤년 경계일 정책은 공개 적격성 API 연결 전에 확인한다.

## 인수 조건

1. 정확히 만 14세가 되는 일반 경계일은 허용하고 하루 전은 차단한다.
2. 서버 기준 시각을 사용자 timezone의 로컬 날짜로 변환해 판정한다.
3. 미래 생년월일과 알 수 없는 timezone을 안전한 도메인 오류로 거부한다.
4. 생년월일 암호문은 같은 입력도 매번 달라야 한다.
5. 다른 사용자 UUID나 변조된 암호문으로 복호화할 수 없어야 한다.
6. 암호화 오류에 생년월일, 키, 암호문을 포함하지 않는다.
7. AES-GCM adapter는 local/test에서만 생성 가능하며 production 구현을 가장하지 않는다.
...
이번 기반 커밋은 공개 endpoint와 request/response field를 추가하지 않는다. 승인 공백 해소 후 기존
`API_CONTRACT.md`의 경로와 오류 envelope를 구현한다.
...
- 평문 생년월일과 만 나이를 로그·분석·agent·decision snapshot으로 전달하지 않는다.
