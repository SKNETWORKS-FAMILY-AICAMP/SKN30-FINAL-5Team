...
- 후보 11건을 INCLUDE 6건, EXCLUDE 5건으로 완전 분할했다.
- 기존 50건과 분리된 증분 시드 6건을 생성해 전체 56건을 구성했다.
- 전체 56건에서 안전 규칙 354건, 목표 보존 대체 관계 238건을 생성했다.
- 기존 6건과 신규 5건을 합친 골든 시나리오 11건이 모두 통과했다.
- 전체 데이터 테스트 121건과 ruff format/check, mypy가 통과했다.
- 안전 규칙·대체 관계·골든 결과를 재생성해 8개 파일의 바이트 동일성을 확인했다.

## 추가 수집 재개 조건

- 현재 카탈로그는 근력 50건, 이동성 4건, 유산소 2건이다. 걷기 외 유산소 유형이나
  추가 이동성 유형이 제품 인수 조건으로 확정되면 해당 유형만 표적 수집한다.
- 사용자 프로필·체크인·수행 결과·미수행 사유·에이전트 협상 로그는 서비스 발생
  데이터이므로 실제 기능과 계약이 준비된 뒤 수집한다.
- 웨어러블 요약, 국민체력100 측정결과, 수면 연구는 현재 MVP 결정에 필수 입력이 아니다.
  관련 기능이 승인되기 전에는 별도 수집하거나 안전 판단 근거로 사용하지 않는다.
- 원천 라이선스 변경, 카탈로그 중복·품질 저하, 사용자 테스트에서 장소·장비·목표 공백이
  확인되면 기존 KSPO·wger 원천 저장분에서 먼저 후보를 재선정하고, 부족할 때만 새 외부
  출처를 추가한다.
