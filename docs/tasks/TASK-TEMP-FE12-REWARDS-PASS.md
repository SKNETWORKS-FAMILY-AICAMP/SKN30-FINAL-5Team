...
6. `CLAIM_DAILY_REWARD` 알림에서 `POST /api/v1/rewards/daily-reward/claim`을 그 자리에서 호출하고, 알림 팝오버를 닫지 않은 채 서버가 지급한 수량을 안내한다. 화면 이동은 하지 않는다. (프로젝트 오너 결정, 2026-09-11. 이전 계획인 "지갑 화면으로 연결한다"를 대체한다.)
