...
- 관련 요구사항: `NFR-002`, `NFR-006`, MVP 기능 `F001`~`F010`, `F025`~`F029`
...
2. 모든 MVP 기능 그룹 `F001`~`F010`, `F025`~`F029`가 API·DB 계약, 코드, 테스트, PR과 연결되며 없는 근거는 `—`로 표시된다.
3. 상태는 `IMPLEMENTED`, `PARTIAL`, `DEFERRED`, `MVP_EXCLUDED` 중 하나로 판정하고 의미를 문서에 정의한다.
4. 웨어러블 `F003`은 구현 완료로 표시하지 않으며, 수동 폴백과 기반 구현 범위를 분리해 기록한다.
