...
- 모든 DB 커넥션은 `lock_timeout` 상한을 갖는다. advisory lock과 행 잠금은 기본값이 무한 대기라,
  커밋하지 못한 보유자 하나가 대기 측 커넥션을 영구히 묶는다. 상한은 정상적인 decision run보다
  충분히 길게 두어 동시성 정책이 아니라 안전장치로만 작동한다(`db_lock_timeout_ms`).
- `async def` 요청 경로는 이벤트 루프에서 블로킹 DB 대기를 수행하지 않는다. 결정 생성과 재생성은
  LLM 호출을 `await`하는 동안 트랜잭션을 연 채로 유지하므로, 잠금 획득을 루프에서 기다리면 그
  잠금을 쥔 요청이 커밋에 도달하지 못해 프로세스 전체가 교착된다. 잠금 획득은 워커 스레드로
  넘기고 세션은 한 번에 한 스레드만 사용한다.
