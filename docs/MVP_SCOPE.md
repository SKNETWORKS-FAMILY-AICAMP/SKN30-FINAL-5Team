...
- 연동 기기·API·출처·품질·라이선스는 후보 탐색 후 개발 전에 확정하며, 권한 거부·미연동 사용자는 수동 체크인 경로를 사용한다.
...
생년월일은 만 18–64세 eligibility 확인을 위해 암호화해 저장한다. 만 나이는 DB에 저장하지 않으며, 서버가 사용자 timezone의 로컬 날짜로 eligibility에 사용한다. 저장된 생년월일과 만 나이는 인증된 본인의 프로필 조회 응답에만 표시하며, 편집 화면이 기존 값을 다시 보여주기 위한 용도다(ADR-0020). 의료적 운동 제한·주의 응답과 범위 밖 연령은 일반 자동 루틴 생성을 차단한다.

생년월일과 만 나이는 로그·분석 이벤트·LLM·에이전트·decision snapshot에
포함하지 않으며, 계정 삭제와 기존 백업 만료 정책을 따른다. 암호화 키 관리와
접근 제한의 구체 구현은 클라우드 확정 후 결정한다.

체중이 입력된 경우에만 앱 운동 계획·세션의 예상 소모 칼로리 추정에 사용할 수 있으며, 체중이 없으면 API·저장 값은 `null`로 둔다. 저장된 체중은 본인 프로필 조회에 반환한다(ADR-0020). 체중·성별은 안전 판정이나 의료적 결론에 사용하지 않는다. 수동 외부 운동 기록과 그에 따른 별도 수기활동 상태는 MVP에서 제외하고 추후 기능으로 분류한다.
...
예상 소모 칼로리는 앱 운동 계획·세션의 운동 종류·운동 시간·강도와 사용자가 제공한 체중을 이용한 추정치로 제공한다. 정밀 칼로리 계산이 아니며 진단 또는 안전 판정의 단독 근거로 사용하지 않는다. 산식과 계수는 버전 관리하고 개발 전 확정한다. 수동 외부 운동 기록 기반의 칼로리 추정은 MVP 이후로 분류한다.
...
14일은 의학적 기준이 아닌 MVP 운영 기준이다. 예정 운동의 `NOT_COMPLETED` 이력과 연속
미수행 횟수는 복귀 트리거가 아니라 비벌점 학습 신호로만 사용한다.
POL-012의 14일 미접속 서비스 분류는 별도 계정 운영 정책이며 운동 복귀 모드를 활성화하지
않는다. 운동 복귀 모드의 유일한 날짜 근거는 마지막 공식 `COMPLETED` 세션이다.

공백 분류:

- 14일 미만: 일반 공백이며 복귀 모드 비활성
- 14일 이상: 복귀 모드로 표시하고 체크인·복귀 상한을 반드시 재평가
...
- 웨어러블 요약 연동 및 수동 체크인 폴백
- 체크인 수동 가능 시간 기반 빈 시간 후보 계산 및 공식 수행 판정과의 분리
