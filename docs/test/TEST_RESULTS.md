...
- 서비스 기능 보강 실행: 2026-09-12, `fix/banana-rewards-and-workout-ux` / PR `#331`
...
> AWS 웹 배포 범위는 실제 Firebase 재로그인·staging API/Aurora 조회와 독립 PostgreSQL 16 CI를 결합해 검증했다. Android/iOS 네이티브 스토어 출시는 별도 실기기 승인 대상이다.

### 서비스 배포 판정과 아키텍처 판정

**현재 배포 대상인 AWS 웹 서비스는 안전성, 핵심 사용자 흐름, 실제 PostgreSQL 통합, Firebase
재로그인, TLS·인증·CORS와 최소 동시 요청 검증을 모두 통과했다. 따라서 MVP 웹 서비스 배포 판정은
`통과`로 한다. 멀티에이전트의 Single-Agent+RAG 대비 비교 우위는 이번 평가에서 입증되지 않았으며,
이는 서비스 배포 판정과 별개다.**
...
아닌 테스트 계약 문제로 판별해 수정했고 최종 736/736건이 통과했다. 로컬 DB 비의존 integration
7건도 통과했다. PR CI에서는 실제 PostgreSQL 16 test DB로 전체 백엔드 2,441건, migration round trip,
V1·V2 release-flow를 모두 통과했다. AWS staging에서 실제 TLS readiness 200, 인증 실패 401, CORS
경계, Google OAuth 재로그인, 홈·프로필·캘린더·저장 운동 상세 조회와 readiness 동시 50건 전부
200을 확인했다. 상세 입력 조건, 기대·실제 결과, 판정과 재현 명령은
`SERVICE_FUNCTIONAL_TEST_RESULTS.md`를 따른다.
...
4. **PHASE 4 표본 14건, 반복 1회**(변동성 측정만 3회). 지표의 신뢰구간을
   말할 수 있는 규모가 아니다.
5. **결합 검증의 환경 분리.** 실제 Firebase 재로그인·배포 API/Aurora 조회는 AWS staging에서,
   PostgreSQL 쓰기·migration·release flow는 독립 CI DB에서 검증했다. 운영 DB 보호를 위해 staging
   schema reset 기반 suite는 실행하지 않았으며, 네이티브 앱은 웹 배포 판정과 분리했다.
...
현재 배포 대상인 AWS MVP 웹 서비스의 전체 판정은 **통과**다. 핵심 사용자 흐름과 안전한 계획
전달, 실제 PostgreSQL CI, Firebase 재로그인, AWS staging 조회 흐름과 공개 API 경계를 확인했다.
Android/iOS 네이티브 스토어 출시는 별도 실기기 승인 대상으로 분리한다. 이 서비스 판정과 아래의
