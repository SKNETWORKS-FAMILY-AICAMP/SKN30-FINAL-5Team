"""Install the completed specialist-agent decision policy version.

Revision ID: 0018_agent_proposal_policy
Revises: 0017_profile_settings
...
revision: str = "0018_agent_proposal_policy"
down_revision: str | None = "0017_profile_settings"
...
def downgrade() -> None:
    op.execute(
        "UPDATE decision_policy_versions SET status_code = 'DEPRECATED' "
        "WHERE version_code = 'decision-policy-v3'"
    )
    op.execute(
