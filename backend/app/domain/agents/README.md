...
- 후보는 `DurationPlan`으로 요청 시간과 정확히 일치해야 하며 Coordinator 입력에 없는 후보나
  운동을 생성할 수 없습니다.
- Coordinator 입력은 `ACTIVE`, `DOMAIN_APPROVED`, `production_eligible=true`, 활성화 완료인
  catalog snapshot만 허용합니다. DRAFT importer 산출물은 입력 계약을 만들 수 없습니다.
...
routine-core adapter는 다음 검증을 마친 후보만 전달합니다.

- `WARMUP -> MAIN -> COOLDOWN` phase와 각 phase의 최소 한 항목
- WARMUP/COOLDOWN의 SUPPORT tier, 하나 이상의 MAIN/CORE, 승인 prescription
- 사용자 가능 장소·장비와 각 운동의 교집합
- `goal_tags`를 추론하지 않고 승인된 `exercise_goal_tag_links`에서 구성
- phase item의 `PlanItemDuration` 합계와 `DurationPlan.warmup_seconds`·
  `cooldown_seconds`의 일치
- setup과 모든 phase item을 포함한 정확한 요청 시간

Coordinator는 이 routine 구성 규칙을 재구현하거나 catalog/ORM을 다시 조회하지 않습니다.
대신 전달된 goal tag, exercise ID, action과 `DurationPlan`을 사용해 proposal 제약과 전체
요청 시간을 검증합니다.

`APPROVED_CATALOG_UNAVAILABLE`, `ROUTINE_DURATION_UNAVAILABLE`,
`ROUTINE_LOCATION_UNAVAILABLE`, `ROUTINE_CONTENT_UNAVAILABLE`은 기본 routine 생성의 공개
HTTP 오류입니다. routine application service가 Coordinator 실행 전에 이 오류를 매핑합니다.
당일 decision에서 승인된 공통 후보가 모두 제외된 경우의 `BLOCKED/REST`와 혼용하지 않습니다.
